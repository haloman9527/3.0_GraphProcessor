...
public abstract class FlowNodeProcessor : BaseNodeProcessor
{
    public FlowNodeProcessor(BaseNode model) : base(model)
    {
        ...
        AddPort(new PortProcessor(ConstValues.FLOW_OUT_PORT_NAME, BasePort.Direction.Right, BasePort.Capacity.Single));
    }
    ...
    {
        foreach (FlowNodeProcessor item in GetPortConnections(ConstValues.FLOW_OUT_PORT_NAME))
        {
            if (item == null)
                continue;
